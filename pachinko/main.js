...
const { Engine, Render, Runner, Bodies, Composite, Events, Mouse, World, Body } = Matter;
...
const render = Render.create({
  ...
  engine,
  options: { width: 500, height: 500, wireframes: false, background: "#000" }
});
...
for (let i = 0; i < pegCount; i++) {
  pegs.push(Bodies.circle(125 + (Math.random()-0.5)*200, 250 + (Math.random()-0.5)*200, 4, { 
    isStatic: true, restitution: 1.1, friction: 0, render: { fillStyle: "#888" } 
  }));
}

for (let i = 0; i < pegCount; i++) {
  pegs.push(Bodies.circle(375 + (Math.random()-0.5)*200, 250 + (Math.random()-0.5)*200, 4, { 
    isStatic: true, restitution: 1.1, friction: 0, render: { fillStyle: "#888" } 
  }));
}
...
document.addEventListener("mousedown", (e) => {
  ...
  if (x < 250) leftAngle = LEFT_UP;
  else rightAngle = RIGHT_UP;
});
